...
pytestmark = [
    pytest.mark.skipif(
        not IT_ENABLED, reason="Hardware integration disabled. Set PNM_CM_IT=1 to run."
    ),
]

# ---- Guard: SNMPv2 must be enabled and SNMPv3 disabled for these tests ----
SNMPV2_OK = bool(SystemConfigSettings.snmp_enable) and not bool(
    SystemConfigSettings.snmp_v3_enable
)
pytestmark.append(
    pytest.mark.skipif(
        not SNMPV2_OK,
        reason="SNMPv2 must be enabled and SNMPv3 disabled for this test suite.",
    )
)
...
def cm() -> CableModem:
    ...
    return CableModem(
        mac_address=mac,
        inet=inet,
        write_community=SystemConfigSettings.snmp_write_community,
    )
