...
- `pytest -q` → pass (565 passed, 4 skipped)

### Tests
- `pytest` → pass (565 passed, 4 skipped)
- `ruff` → pass
- `python3 -m compileall src` → pass
...
class MultiDsChanEstRouter(AbstractService):
    ...
    def _add_routes(self) -> None:
        ...
        def analysis(
            ...
            """
            try:
                capture_group_id = OperationManager.get_capture_group(
                    request.operation_id
                )
            except Exception as exc:
                self.logger.error(
                    "[analysis] Failed to resolve capture group for operation_id=%s: %s",
                    request.operation_id,
                    exc,
                )
