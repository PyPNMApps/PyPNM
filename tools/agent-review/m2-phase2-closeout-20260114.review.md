...
- Goal: Align Phase 2 bootstrap docs and tests to the schema_meta-only contract and ensure Postgres integration hooks are gated by explicit env flags.
- Changes: Clarified bootstrap-only wording in database backend design doc; updated schema_meta seeding for Postgres and SQLite stats usage in tests; added PYPNM_TEST_POSTGRES gating to Postgres integration tests across DB schema manager and repository tests.
- Files: docs/design/db/database-backend.md, src/pypnm/db/schema_version.py, src/pypnm/db/sqlite_adapter.py, src/pypnm/db/postgres_adapter.py, tests/test_database_manager.py, tests/test_db_schema_manager.py, tests/test_capture_group_repository.py, tests/test_session_group_repository.py, tests/test_operation_capture_repository.py.
- Tests: python3 -m compileall src; ruff check .; ruff format --check .; pytest -q.
- Notes: pytest skips include hardware integration (PNM_CM_IT), Postgres DSN-dependent tests, and PYPNM_TEST_POSTGRES-gated integration hooks.
