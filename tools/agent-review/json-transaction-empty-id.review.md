...
- tests/test_json_transaction_db_empty_transaction_id.py

### Commands Executed And Results
- `python3 -m compileall src` → pass
- `ruff check .` → pass
- `ruff format --check .` → pass
- `pytest -k "json_transaction_db and empty_transaction" -q` → pass (3 passed, 540 deselected)
...
### Notes / Warnings
- Pytest emits expected warnings for missing JSON DB file prior to first write.
...
class JsonTransactionDb:
    ...
    def write_json(
        ...
        * Returns the updated ``JsonTransactionDbModel``.
        ...
        if not tx_id.strip():
            self.logger.warning("Skipping DB insert for empty transaction_id")
            return self._load_db_model()
    ...
    def _load_db_model(self) -> JsonTransactionDbModel:
        ...
        for tx_id, payload in obj.items():
            if not str(tx_id).strip():
                self.logger.warning("Skipping empty transaction_id in JSON DB")
                continue
