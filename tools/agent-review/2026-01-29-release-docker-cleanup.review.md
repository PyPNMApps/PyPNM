...
DOCKER_SMOKE_IMAGE: Final[str]           = "pypnm:latest"
...
def _cleanup_docker_image(image: str, use_sudo: bool) -> None:
    cmd = ["docker", "image", "rm", image]
    label = "docker-image-cleanup"
    if use_sudo:
        cmd = ["sudo"] + cmd
        label = "docker-image-cleanup-sudo"
    _run(cmd, check=False, label=label)
...
def main() -> None:
    ...
    if not skip_docker:
        ...
        _cleanup_docker_image(DOCKER_SMOKE_IMAGE, used_sudo)
    else:
