...
- Goal: Remove redundant session-group index and tighten Option B membership docstrings.
- Changes: Dropped idx_sg_tx_session_id in SQLite/Postgres schemas; clarified SessionGroup and repository docstrings; formatting-only spacing fix in repository.
...
- Tests: python3 -m compileall src; ruff check src; ruff format --check .; pytest -q (590 passed, 4 skipped).
