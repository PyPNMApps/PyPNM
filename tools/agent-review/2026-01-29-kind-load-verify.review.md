...
- Goal: Stabilize kind image loading in local smoke/k8s toolkit by verifying image presence and retrying on timeouts.
- Changes: Add load verification via crictl and retry loop with configurable env vars.
- Files: tools/local/local_kubernetes_smoke.sh, tools/k8s/pypnm_k8s_toolkit.sh
- Tests: python3 -m compileall src; ruff check src; ruff format --check . (fails: repo drift); pytest -q (3 skipped: PNM_CM_IT)
- Notes: Review bundle regenerated after adding image verification/retry.

# FILE: tools/local/local_kubernetes_smoke.sh
#!/usr/bin/env bash
...
# FILE: tools/k8s/pypnm_k8s_toolkit.sh
#!/usr/bin/env bash
set -euo pipefail
...
if [[ "${IMAGE_SOURCE}" == "local" ]]; then
  ...
  if ! load_image_into_kind "pypnm:local" "${CLUSTER_NAME}"; then
    echo "Failed to load image into kind cluster ${CLUSTER_NAME}." >&2
    exit 1
  fi
