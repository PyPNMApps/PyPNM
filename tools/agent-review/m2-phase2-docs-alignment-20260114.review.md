...
- Goal: Align Phase 2 docs with the implemented baseline schema bootstrap and confirm Ruff policy matches staged enablement notes.
- Changes: Updated bootstrap-contract and database-backend docs to describe baseline schema initialization and seeding; reverted Ruff rule selection to core families per staged enablement notes.
- Files: pyproject.toml, docs/design/db/bootstrap-contract.md, docs/design/db/database-backend.md.
- Tests: python3 -m compileall src; ruff check .; ruff format --check .; pytest -q.
- Notes: ruff check reports existing import ordering issues in src/pypnm/api/routes/common/classes/file_capture/pnm_file_transaction.py and src/pypnm/api/routes/docs/pnm/files/service.py. pytest skips include PNM_CM_IT hardware integration and PYPNM_TEST_POSTGRES-gated Postgres tests.
