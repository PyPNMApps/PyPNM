...
- Goal: Make DB schema assets installable from package data and align bootstrap-only docs/config/testing with guarded Postgres usage.
- Changes: Added packaged schema SQL assets and importlib.resources loading; updated schema manager tests to read package data; gated Postgres integration tests behind PYPNM_TEST_POSTGRES; refreshed bootstrap docs to note package assets and config alignment; updated package-data to include SQL files.
- Files: pyproject.toml, src/pypnm/lib/db/db_schema_manager.py, src/pypnm/db/schema/__init__.py, src/pypnm/db/schema/sql/__init__.py, src/pypnm/db/schema/sql/schema_postgres.sql, src/pypnm/db/schema/sql/schema_sqlite.sql, tests/test_db_schema_manager.py, tests/test_capture_group_repository.py, tests/test_operation_capture_repository.py, tests/test_session_group_repository.py, docs/design/db/database-backend.md, docs/design/db/bootstrap-contract.md.
- Tests: python3 -m compileall src; ruff check .; ruff format --check .; pytest -q.
- Notes: pytest skips include hardware integration (PNM_CM_IT) and Postgres integration hooks gated by PYPNM_TEST_POSTGRES plus DSN/psycopg.
...
# FILE: src/pypnm/db/schema/__init__.py
# SPDX-License-Identifier: Apache-2.0
# Copyright (c) 2026 Maurice Garcia

from __future__ import annotations

# FILE: src/pypnm/db/schema/sql/__init__.py
# SPDX-License-Identifier: Apache-2.0
# Copyright (c) 2026 Maurice Garcia

from __future__ import annotations

# FILE: src/pypnm/db/schema/sql/schema_postgres.sql
-- SPDX-License-Identifier: Apache-2.0
-- Copyright (c) 2026 Maurice Garcia
...
# FILE: src/pypnm/db/schema/sql/schema_sqlite.sql
-- SPDX-License-Identifier: Apache-2.0
-- Copyright (c) 2026 Maurice Garcia
...
If Postgres is selected but the DSN is invalid or blank, the adapter selection falls back to SQLite without raising.
...
raising from get_adapter. Direct configuration validation may still raise ValidationError.
