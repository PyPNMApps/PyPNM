...
### Summary
Implemented backend env overrides in SystemConfigSettings, documented PYPNM_DB_BACKEND, and ensured settings templates exist for demo and app roots. Updated install.sh to always print backend guidance after selection and added tests for backend env override.
...
- demo/settings/system.json.template
- src/pypnm/settings/system.json.template

### Commands Executed And Results
- `python -m compileall src` → failed (python not found)
- `python3 -m compileall src` → passed
- `pytest` → passed (513 passed, 3 skipped)
- `ruff check .` → failed (pre-existing repo violations)
- `ruff format --check .` → failed (pre-existing formatting differences)

### Tests
- `pytest` → passed (513 passed, 3 skipped)
- `ruff` → failed (pre-existing repo violations)
- `ruff format --check .` → failed (pre-existing formatting differences)

### Notes / Warnings
- python binary not available as `python`; used `python3` for compileall.
- Ruff failures appear pre-existing and not introduced by this change set.
...
# FILE: /home/dev01/Projects/PyPNM/demo/settings/system.json.template
{
    "FastApiRequestDefault": {
        "mac_address": "aa:bb:cc:dd:ee:ff",
...
# FILE: /home/dev01/Projects/PyPNM/src/pypnm/settings/system.json.template
{
    "FastApiRequestDefault": {
        "mac_address": "aa:bb:cc:dd:ee:ff",
        "ip_address": "192.168.0.1"
