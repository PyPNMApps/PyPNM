...
- Goal: Add a Postgres-only FK integrity regression test mirroring the SQLite check.
- Changes: Added Postgres FK integrity test with DSN/psycopg skip gates and a helper to seed transaction_records on Postgres.
...
def _insert_transaction_postgres(dsn: DatabaseDsn, transaction_id: str) -> None:
...
def test_session_group_repository_enforces_fk_integrity_postgres(
    ...
    if not SystemConfigSettings.database_postgres_dsn():
    ...
    monkeypatch.setattr(
        SystemConfigSettings,
        "database_sqlite_path",
        classmethod(lambda cls: DatabasePath("")),
    )
    monkeypatch.setattr(
        SystemConfigSettings,
        "database_postgres_dsn",
    ...
    repo = SessionGroupRepository.from_system_config()
    session_id = GroupId("session-fk-postgres")
    repo.create_session_group(session_id, TimestampSec(DEFAULT_CREATED_EPOCH))
    ...
    _insert_transaction_postgres(postgres_dsn, "missing-txn")
    repo.add_transaction(
        session_id,
        TransactionId("missing-txn"),
        TimestampSec(DEFAULT_ADDED_EPOCH),
