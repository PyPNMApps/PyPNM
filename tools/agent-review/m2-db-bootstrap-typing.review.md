...
- Goal: Tighten DB bootstrap test typing and add a minimal schema_version inspection hook without expanding schema scope.
- Changes: Added SQLite adapter schema_version_stats helper, updated SQLite seed test to use it, and generalized fake cursor typing.
- Files: src/pypnm/db/sqlite_adapter.py; tests/test_database_manager.py.
...
class SQLiteDatabaseAdapter(DatabaseAdapter):
    ...
    def schema_version_stats(self) -> tuple[int, int]:
        self._ensure_connection()
        if self._connection is None:
            return 0, 0
