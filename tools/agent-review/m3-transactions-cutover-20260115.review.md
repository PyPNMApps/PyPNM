...
- Goal: Enforce DB-authoritative transactions cutover guardrails and remove legacy ledger reads at runtime and in tests.
- Changes: Added a shared ledger-read guard for tests; refactored file service tests to use it; ensured JSON export cannot write `transactions.json` and uses canonical settings; updated Phase 3 purpose wording; updated SPDX years on touched code files.
- Files: src/pypnm/lib/db/db_schema_manager.py; src/pypnm/lib/db/json_transaction.py; src/pypnm/tools/migrate_transactions.py; tests/ledger_guard.py; tests/test_migrate_transactions.py; tests/test_pnm_file_artifact_resolution.py; tests/test_pnm_file_hexdump.py; docs/design/db/database-backend.md
- Tests: python3 -m compileall src; ruff check .; ruff format --check .; pytest -q
- Notes: pytest skips include PNM_CM_IT hardware integration and Postgres gating via PYPNM_TEST_POSTGRES; tools/agent-review contains planning dirs (including a trailing-space sibling) left untouched.
...
PyPNM historically persisted transaction metadata using JSON “ledger” files under `.data/db/`. As of Phase 3,
the DB is authoritative for transaction metadata, and the JSON ledgers are deprecated (legacy-only, migration
use). Capture groups and operations may remain staged for Phase 4+, but transaction reads and writes now use the
DB as the system of record. This design defines the target state for replacing the ledger with a relational
database while preserving PyPNM’s operational model (filesystem-based binary artifacts plus lightweight metadata
persistence).
...
- Transactions legacy import is supported via `python -m pypnm.tools.migrate_transactions --input PATH`.
  If `--input` is omitted, the migrator looks for `.data/db/transactions.json` and logs a warning (no-op) when missing.
  Invalid JSON produces a clear error and non-zero exit code.
- As of Phase 3, the DB is authoritative for transaction metadata and `transactions.json` is legacy-only.
