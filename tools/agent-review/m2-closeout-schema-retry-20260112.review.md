...
- Goal: Align runtime schema with documented DDL, harden capture-group retry allocation with single-connection attempts, and clean DB authority wording in docs.
- Changes: Added connection-aware retry helpers with FK/unique handling, added deprecated db_path warning, added SQLite/Postgres index presence tests, removed SPDX header from database-backend doc and updated cutover language, and ensured capture-group/operation indexes are documented and present in DDL.
- Files: src/pypnm/lib/db/capture_group_repository.py, src/pypnm/api/routes/common/classes/file_capture/capture_group.py, tests/test_capture_group_repository.py, tests/test_db_schema_manager.py, docs/design/db/database-backend.md, docs/design/db/schema_sqlite.sql, docs/design/db/schema_postgres.sql
