...
- Goal: Restore green Ruff lint by fixing import ordering and confirm DB backend selection contract remains consistent.
- Changes: Reordered first-party imports in PNM file transaction and file service modules to match Ruff I rules; confirmed DatabaseManager fallback on ValidationError and fail-fast schema manager behavior without doc changes.
- Files: src/pypnm/api/routes/common/classes/file_capture/pnm_file_transaction.py, src/pypnm/api/routes/docs/pnm/files/service.py.
