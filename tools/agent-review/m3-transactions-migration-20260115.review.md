...
- Goal: Add an explicit, idempotent transactions.json migrator and document migration usage for Phase 3.
- Changes: Added a transaction migrator CLI that imports legacy records into DB with duplicate/invalid handling; added unit tests for import, idempotence, missing/invalid input, and skipping malformed entries; documented the migrator usage and default path in the migration strategy section.
- Files: src/pypnm/tools/migrate_transactions.py; tests/test_migrate_transactions.py; docs/design/db/database-backend.md.
- Tests: python3 -m compileall src; ruff check .; ruff format --check .; pytest -q.
- Notes: Expected skips include cm_it hardware integration and Postgres tests gated by PYPNM_TEST_POSTGRES.
...
class TransactionMigrator:
    ...
    def _resolve_default_path() -> Path:
        app_root = DatabaseSchemaManager.from_system_config()._resolve_app_root()
