...
- Goal: Align DB bootstrap documentation with schema-only implementation and close M2 config/precedence coverage gaps.
- Changes: Added bootstrap contract doc, clarified planned schema scope in design doc, and expanded tests for config precedence and initialize failure paths.
- Files: docs/design/db/bootstrap-contract.md; docs/design/db/database-backend.md; tests/test_system_config_settings.py; tests/test_database_manager.py.
...
If Postgres is selected but the DSN is invalid or blank, selection falls back to SQLite without raising.

## Schema Bootstrap Scope

The bootstrap schema is limited to `schema_version`:

- `apply_schema` ensures `schema_version` exists and is seeded once.
...
- `schema_version` exists solely to verify minimal DB readiness.
- `initialize()` connects, applies the schema version seed if empty, and checks health.
...
def test_sqlite_adapter_apply_schema_seeds_version(tmp_path: Path) -> None:
    ...
    try:
        ...
        adapter.apply_schema()
        count, minimum = adapter.schema_version_stats()
        assert count == 1
