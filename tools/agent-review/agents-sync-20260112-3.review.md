...
- Goal: Sync AGENTS.md to align SPDX year guidance with Repo Hygiene rules.
- Changes: Clarified header year rule to allow single year or range per Repo Hygiene policy.
- Files: AGENTS.md.
- Tests: python3 -m compileall src; ruff check src; ruff format --check .; pytest -q (600 passed, 9 skipped).
- Notes: Initial pytest run timed out at 10s; reran with extended timeout. Skips: PNM_CM_IT hardware integration and PYPNM_DB_POSTGRES_DSN-gated tests.
