...
- Goal: Fix Postgres FK test gating and make exception assertion robust to psycopg integrity errors.
- Changes: Removed pre-monkeypatch DSN skip, used temp sqlite placeholder, adjusted helper signature, and switched to psycopg.IntegrityError.
...
def test_session_group_repository_enforces_fk_integrity_postgres(
    ...
    _insert_transaction_postgres(postgres_dsn, sqlite_placeholder, "missing-txn")
    repo.add_transaction(
        session_id,
        TransactionId("missing-txn"),
        TimestampSec(DEFAULT_ADDED_EPOCH),
