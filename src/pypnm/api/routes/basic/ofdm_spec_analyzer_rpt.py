...
class OfdmSpecAnalyzerAnalysisReport:
    ...
    def to_model(self) -> OfdmSpecAnalysisRptModel:
        ...
        """
        return OfdmSpecAnalysisRptModel(models=self._multi_analysis.to_model())
        ...
        for common_model in self.get_common_analysis_model():
            ...
            try:
                fname = self.create_png_fname(
                    tags=[str(channel_id), self.FNAME_TAG, "standard"]
                )
                self.logger.debug(
                    f"Creating Standard OFDM Channel ({channel_id})Spectrum Plot: %s",
                    fname,
                )
            ...
            try:
                fname = self.create_png_fname(
                    tags=[str(channel_id), self.FNAME_TAG, "moving_average"]
                )
                self.logger.debug(
                    "Creating OFDM Moving Average Spectrum plot: %s", fname
                )
